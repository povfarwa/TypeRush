...
function updateLiveStats() {
  const elapsed = state.selectedTime - state.timeLeft;
  if (elapsed <= 0) return;
  ...
  dom.progressBar.style.width = Math.min(pct, 100) + '%';
}
...
function handleInput() {
  ...
  placeCursor(state.wordIndex, state.charIndex);
  updateLiveStats();
}
...
function showResults() {
  ...
  const accNum     = (totalCorrect + totalErrors) > 0
...
dom.ghostInput.addEventListener('blur',  () => {
  if (!state.isFinished) setFocus(false);
});
...
dom.retryBtn.addEventListener('click',   resetTest);
